    ...
    if show_new_section and report_data.get("new_titles", []):
        ...
        for source_data in report_data["new_titles"]:
            for title_data in source_data["titles"]:
                ...
                # 生成纯文本标题（标题 + URL），钉钉会自动识别 URL 为可点击链接
                formatted_title = format_title_for_platform(
                    "dingtalk", title_data_copy, show_source=False, plain_text=True
                )
    ...
    if show_new_section and report_data.get("new_titles", []):
        ...
        for source_data in report_data["new_titles"]:
            for title_data in source_data["titles"]:
                ...
                title_data_copy = title_data.copy()
                title_data_copy["is_new"] = False
                # 生成纯文本标题（标题 + URL），飞书会自动识别 URL 为可点击链接
                formatted_title = format_title_for_platform(
                    "feishu", title_data_copy, show_source=False, plain_text=True
                )
                # 去重：使用原始标题文本（不含 URL）作为去重键
                title_text = clean_title(title_data_copy["title"])
                if title_text not in seen_titles:
